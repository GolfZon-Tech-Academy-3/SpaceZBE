...
public class CompanyS3Service {
    ...
    // S3 bucket에 image upload 후, Image name 반환
    ...
    public String update(Long companyId, MultipartFile file) {
        String imageUrl;
        List<String> imageUrls = new ArrayList<>(Arrays.asList(profileImages));
        Optional<Company> company = companyRepository.findById(companyId);
        if (company.isPresent()) {
            String savedImage = company.get().getImageName().replace("https://spacez3.s3.ap-northeast-2.amazonaws.com/", "");
